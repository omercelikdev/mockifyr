...
/// <summary>
/// Validation of change-feed reload (G16e): the last G16 slice. Two live Mockifyr hosts share one
/// Redis backend with <c>--change-feed</c>; a stub created (or deleted) over the admin API on one
/// instance is served (or stopped) by the other <em>without a restart</em> — multi-instance coherence.
/// Propagation is asynchronous (Redis pub/sub), so the assertions poll within a timeout. This is
/// coherence infrastructure (no WireMock-semantic novelty; served-response parity is covered by G16d),
/// so no oracle is needed. Requires Docker.
/// </summary>
public sealed class G16eChangeFeedTests : IAsyncLifetime
{
    ...
    // Polls until the endpoint returns the expected status, or a generous timeout elapses (pub/sub
    ...
    {
        var app = MockifyrHost.Build(["--port", "0", "--redis", connectionString, "--change-feed", "true"]);
